...
namespace QuanLyKhachSan.Daos
{
    public class RoomDao
    {
        ...
        public List<Room> GetRoomTop5(int hotelId)
        {
            return myDb.rooms.Where(x => x.HotelId == hotelId).OrderByDescending(x => x.view).Take(3).ToList();
        }
        ...
        public List<Room> GetRoomDiscount(int hotelId)
        {
            return myDb.rooms.Where(x => x.HotelId == hotelId && x.discount > 0).OrderByDescending(x => x.discount).Take(3).ToList();
        }
        ...
        {
            return myDb.rooms.Where(x => x.HotelId == hotelId).ToList();
        }
        ...
        public List<Room> GetRoomsBlank(int hotelId, int page, int pagesize)
        {
            var arrIdRoom = myDb.bookings.Where(x => x.status == 0 || x.status == 1).Select(x => x.idRoom).Distinct().ToList();
            var allId = myDb.rooms.Where(x => x.HotelId == hotelId).Select(x => x.idRoom).ToList();
            var ids = allId.Except(arrIdRoom).ToList();
            return myDb.rooms.Where(x => ids.Contains(x.idRoom)).ToList().Skip((page - 1) * pagesize).Take(pagesize).ToList();
        }
        public List<Room> GetRoomsBlank(int hotelId, int page, int pagesize,DateTime startDate, DateTime date)
        {
            var arrIdRoom = myDb.bookings.Where(x => x.status == 0 || x.status == 1).Select(x => x.idRoom).Distinct().ToList();
            var allId = myDb.rooms.Where(x => x.HotelId == hotelId).Select(x => x.idRoom).ToList();
            var ids = allId.Except(arrIdRoom).ToList();
        ...
        public List<Room> SearchByCity(int page, int pagesize, int cityId, int numberChildren, int numberAdult)
        {
            var query = (from b in myDb.hotels
                         join c in myDb.rooms on b.HotelId equals c.HotelId
                         where b.CityId == cityId && c.numberAdult >= numberAdult && c.numberChildren >= numberChildren
                         select c
                         ).ToList();
            /*var arrIdRoom = myDb.bookings.Where(x => x.status == 0 || x.status == 1).Select(x => x.idRoom).Distinct().ToList();
            var allId = myDb.rooms.Select(x => x.idRoom).ToList();
